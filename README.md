...
Let us suppose that `node_1` needs to download some `data_x`, owned by `node_2`, and pay for the fees required by `node_2`. What happens in the case of Byzantine fault nodes? How can the network solve the following:
...
Verifiers use Merkle Trees that are generated by participating nodes as an efficient data verification mechanism without having the actual data content. The verifier can verify if data has been transferred by comparing these Merkle trees.
